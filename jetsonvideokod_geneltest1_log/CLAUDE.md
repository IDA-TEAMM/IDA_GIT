...
- **Etkin menzil ≈15 m (F5.5 sözleşme):** `min_area_px=150`'den türetilir
  (30 cm duba, 640 px / 1.2 rad HFOV → alan=150 ⇒ d≈15 m; iki yanı
  `scene_camera_menzil_siniri` testiyle kanıtlı). LiDAR menzili 25 m →
  **15-25 m bandındaki duba yalnız LiDAR'da görünür** = füzyonda
  `CLASS_UNKNOWN(99)` engel olarak KORUNUR (güvenlik; renk sınıfı ancak
  ≲15 m'de gelir). Menzil artırma = `min_area_px` düşürme (yanlış-pozitif
  pahasına — bilinçli takas, sahada karar).
- **`score` semantiği (F5.6 sözleşme — güven skoru DEĞİL):** kaynağa bağlı:
  HSV kontur yolu → **doluluk oranı** `area/(w·h)` (şekil metriği; daire
  ≈0.785, dikdörtgen şerit 1.0 — yani turuncu halat/şerit dubadan YÜKSEK
  skor alır, `scene_camera_turuncu_serit` testinde görünür); YOLO katmanı →
  ağ güveni. **Tüketici bu alanı güven eşiği olarak KULLANAMAZ** (2026-07-15
  grep teyidi: planning/fusion hiçbir kararı bu alana bağlamıyor; fusion
  yalnız pass-through). Kaynaklar arası karşılaştırma anlamsız.
