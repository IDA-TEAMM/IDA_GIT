...
# ----------------------------------------------- F-M.4: mod isteği hız sınırı

def _fm4_bridge(interval: float) -> MavrosBridge:
    b = MavrosBridge(MavrosBridgeConfig(mode_retry_interval_s=interval))
    b.set_mission_state("PARKUR1")
    b.update_state(0.0, connected=True, armed=True, guided=False, mode="HOLD")
    return b


def test_fm4_mode_request_araligi_dolmadan_tekrar_istenmez() -> None:
    """F-M.4: ArduPilot GUIDED'ı reddedince (ör. GPS fix yok) her state
    mesajında (F-M.6 sonrası 10 Hz) yeni istek atılıyordu — FCU'ya istek
    yağmuru + log spam'i. İstekler arası asgari bekleme uygulanır."""
    b = _fm4_bridge(2.0)
    assert b.should_request_mode(0.0) is True            # ilk istek hemen
    b.note_mode_requested(0.0)
    assert b.should_request_mode(0.1) is False           # 10 Hz state → sustur
    assert b.should_request_mode(1.9) is False
    assert b.should_request_mode(2.0) is True            # aralık doldu → tekrar


def test_fm4_interval_sifir_eski_davranis() -> None:
    """0 → sınır kapalı (geri uyumluluk)."""
    b = _fm4_bridge(0.0)
    b.note_mode_requested(0.0)
    assert b.should_request_mode(0.1) is True


def test_fm4_mod_duzelince_istek_kesilir() -> None:
    """Hız sınırı needs_mode_change'i EZMEZ: mod hedefe dönünce istek yok."""
    b = _fm4_bridge(2.0)
    b.note_mode_requested(0.0)
    b.update_state(3.0, connected=True, armed=True, guided=True, mode="GUIDED")
    assert b.should_request_mode(5.0) is False
