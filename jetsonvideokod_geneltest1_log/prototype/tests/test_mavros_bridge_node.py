...
def test_fm4_mod_istek_spami_hiz_sinirli(ros_context) -> None:  # noqa: ANN001
    """F-M.4 repro: görev aktif + mod != GUIDED + FC reddediyor senaryosunda
    10 Hz state akışı her mesajda yeni istek üretiyordu. Artık istekler arası
    asgari `mode_retry_interval_s` (2 s) beklenir."""
    n = girdap.MavrosBridgeNode()
    try:
        t = [100.0]
        n._now = lambda: t[0]
        calls: list[float] = []
        n._request_guided = lambda: calls.append(t[0])
        n._auto_guided = True
        n._bridge.set_mission_state("PARKUR1")

        for _ in range(5):                       # 0.5 s'lik 10 Hz penceresi
            n._on_state(_state(armed=True))      # mode=MANUAL ≠ GUIDED
            t[0] += 0.1
        assert len(calls) == 1, f"0.5 s'de {len(calls)} istek (spam, F-M.4)"

        t[0] += 2.0                              # aralık doldu
        n._on_state(_state(armed=True))
        assert len(calls) == 2, "aralık dolunca yeniden istenmedi"
    finally:
        n.destroy_node()
