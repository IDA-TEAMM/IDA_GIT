...
# Her _exchange'e süreç-benzersiz, sabit test stamp'lerinden (0/555/987) UZAK bir
# sec ver → aynı süreçte başka bir testin (ör. perception_camera_node testinin
# buoys stamp (0,456) mesajı) DDS'te bıraktığı BAYAT mesaj, slop (0.3 s) içine
# düşüp ApproximateTimeSynchronizer'ı kirletemez. Grup koşusunda test_synced'i
# flaky yapan kök neden: iki üreticinin de sec=0 kullanması → yanlış eşleşme.
# Gerçek dağıtımda sensörler sürekli ARTAN stamp üretir; bu onu taklit eder.
_EXCHANGE_STAMP_SEQ = itertools.count(10_000)
...
def _exchange(
    ...
    """İki mesajı (AYNI, benzersiz stamp) bas, classified_obstacles bekle.

    stamp_sec=None → süreç-benzersiz büyük bir sec üretir (_EXCHANGE_STAMP_SEQ):
    aynı süreçteki başka testlerin DDS'te bıraktığı bayat mesajlar slop içine
    düşüp yanlış eşleşme üretemez. Belirli stamp gereken test (LiDAR referans
    stamp'i korunuyor mu?) değeri açıkça verir.
    ...
    if stamp_sec is None:
        stamp_sec = next(_EXCHANGE_STAMP_SEQ)
    # İki mesaja da AYNI benzersiz stamp'i bas (senkronizasyon eşleşmesi için
    # zorunlu); bayat cross-test mesajları bu stamp'e slop içinde denk gelemez.
    for _m in (pose_msg.header, det_msg.header):
        _m.stamp.sec = stamp_sec
        _m.stamp.nanosec = 123
