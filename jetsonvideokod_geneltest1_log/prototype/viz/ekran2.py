...
def find_latest_graph_csv(directory: Union[str, Path]) -> Path:
    """En yeni grafik CSV'si — DOSYA ZAMANINA göre.

    16.07 kayıt düzeni: adaylar `<N>/grafik.csv` (her kayıt kendi numaralı
    klasöründe) + geçiş için eski düzen `grafik_*.csv`. Ada göre sıralama
    YAPILAMAZ: kayıt numarası silinen numarayı yeniden kullanır (Eyüp kuralı)
    → en büyük numara ≠ en yeni kayıt.
    """
    directory = Path(directory).expanduser()
    candidates = list(directory.glob("*/grafik.csv"))
    candidates += directory.glob("grafik_*.csv")
    if not candidates:
        raise FileNotFoundError(
            f"{directory} içinde */grafik.csv ya da grafik_*.csv yok"
        )
