...
class MissionManagerNode(Node):
    ...
    def __init__(self, **node_kwargs) -> None:
        ...
        # F-V.8: FC seq → bizim index eşlemesi (fc modu) + reached tek-atış
        # tekrarı önleyen işaret (kendi varışımız ve FC sinyali aynı index'i
        # iki kez yayınlamasın — parkur katmanı Int32 tüketiyor).
        self._fc_seqs: List[int] = []
        self._last_reached_pub = -1
    ...
    def _setup_fc_source(self) -> None:
        ...
        # F-V.8: FC'nin kendi varış sinyali (AUTO'da görev ilerlemesinin tek
        # dürüst kaynağı). BİLEREK volatile (depth 10) abonelik: yayıncı
        # TRANSIENT_LOCAL — latched abone olsaydık BİR ÖNCEKİ koşunun son
        # 'reached' mesajı boot'ta tekrar gelirdi (bayat mesaj görevi sahte
        # ilerletirdi; notify_external_reached IDLE koruması da var).
        # GUIDED yarışmasında FC görev koşmaz → topic hiç akmaz → etkisiz.
        ...
        self.get_logger().info(
            ...
            f"(skip_home_seq0={self._skip_home}) + FC varış senkronu aktif "
    ...
    def _on_fc_waypoints(self, msg) -> None:                      # noqa: ANN001
        ...
        wps, seqs = fc_items_to_waypoints_with_seqs(
            items, skip_home_seq0=self._skip_home
        )
        if not wps:
            self.get_logger().warn(
    ...
    def _on_fc_reached(self, msg) -> None:       # mavros_msgs/WaypointReached
        """F-V.8: FC 'waypoint'e vardım' dedi → index'imizi ileri senkronla.

        FC'nin WP_RADIUS'u bizim arrival_radius_m'den farklıysa (ya da rover
        köşeyi yarıçapımıza girmeden dönerse) kendi varış tespitimiz TAKILIR:
        yon_setpoint geriyi gösterir, COMPLETE gelmez, FSM PARKUR1'de kalır.
        FC görev otoritesi olduğuna göre (AUTO) onun sinyali kazanır.
        """
        try:
            idx = self._fc_seqs.index(int(msg.wp_seq))
        except ValueError:
            return                               # home/DO item ya da eşleme yok
        if not self._started:
            self.get_logger().warn(
                f"FC reached (seq={msg.wp_seq}) ama görev başlamadı — yok sayıldı",
                throttle_duration_sec=5.0,
            )
            return
        if not self._mgr.notify_external_reached(idx):
            return                               # geride/bitmiş — zaten işlendi
        self.get_logger().info(
            f"FC varış senkronu: wp_seq={msg.wp_seq} → index {idx} "
            + ("(GÖREV TAMAM)" if self._mgr.is_complete else
               f"→ yeni hedef {self._mgr.current_index}")
        )
        self._publish_reached_once(idx)

    def _publish_reached_once(self, idx: int) -> None:
        """waypoint_reached tek-atış yayını — aynı index iki kez basılmaz
        (kendi varışımız + FC sinyali yarışabilir; parkur katmanı Int32 tüketir)."""
        if idx <= self._last_reached_pub:
            return
        self._last_reached_pub = idx
        self._pub_reached.publish(Int32(data=idx))

    # ----- yayım -----

    def _now(self) -> float:
        # F-M.10: dwell süresi GÖRELİ — duvar saati sıçrayınca 30 s dwell
        # "anında doldu" olur, waypoint sahte ilerlerdi. Damga (header.stamp)
        # duvar saatinde kalır.
    ...
    def _on_tick(self) -> None:
        ...
        if phase is MissionPhase.DWELL and self._prev_phase is not MissionPhase.DWELL:
            ...
            self._publish_reached_once(idx)      # F-V.8: FC sinyaliyle yarışabilir
