...
class FusionNode(Node):
    ...
    def _now(self) -> float:
        # F-M.10: F8.2 bekçisinin girdi yaşı GÖRELİ süre — duvar saati NTP
        # sıçramasında yaş gibi görünür (odom yayını sahte kesilirdi).
        return time.monotonic()

    def _mark_input(self) -> None:
        """F8.2: poz kaynağını süren bir girdi geldi — bayatlık saatini sıfırla."""
        self._last_input_t = self._now()
    ...
    def _on_publish_timer(self) -> None:
        ...
        if self._pose_timeout_s > 0.0 and self._last_input_t is not None:
            age = self._now() - self._last_input_t
