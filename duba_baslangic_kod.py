...
import time 
#PHYTON KOPYALA YAPISTIR YAPARSANIZ DAHA RAHAT ANLASILIR
...
class  IdaAlgilamaDugumu(Node):#iletişim yetenekleri(measj,gönderme,veri tutma)
    ...
    def tespit_callback(self, msg):
        ...
        if self.tespit_edilenler:
            son = self.tespit_edilenler[-1]
            fark = ((son.x-yeni_duba.x)**2+(son.y-yeni_duba.y)**2)**0.5#ne kadar yer değiştirdi
            if fark< 0.10:
                return
