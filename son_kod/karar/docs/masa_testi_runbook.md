# Masa Testi Runbook'u — Jetson + Pixhawk + MicoAir + QGC
...
## M2 — QGC ↔ MicoAir ↔ Pixhawk (kablosuz YKİ)

MicoAir biri Pixhawk TELEM portunda, eşi laptopta USB. QGC'yi aç.
- QGC otomatik bağlanmazsa: Application Settings → Comm Links → seri link
  ekle (telemetri modülünün COM portu, 57600) → Connect.
- **PASS (3 alt madde):**
  1. QGC HUD'da telemetri canlı (mod, batarya, GPS).
  2. QGC'den **Arm** → M1'deki `ros2 topic echo /mavros/state` içinde
     `armed: true` görünür (komut telemetri→FCU→USB→MAVROS zincirini doğrular).
     Hemen Disarm et.
  3. QGC'den mod değiştir (ör. Manual→Hold) → `/mavros/state` `mode:`
     alanında değişim görünür.
- **FAIL:** MicoAir LED'leri eşleşme durumunu gösterir; baud/NetID eşleşmesini
...
| M2 QGC/MicoAir | ⏭ ERTELENDİ | QGC laptop yok; QGC'nin ARM64 sürümü yok (Jetson'a kurulamaz) |
