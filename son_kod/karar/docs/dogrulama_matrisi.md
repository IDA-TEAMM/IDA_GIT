...
| QGC↔MicoAir↔Pixhawk kablosuz hat (M2) | MicoAir çifti kurulumu + QGC laptop + Pixhawk |
...
1. **Ön şart:** pervaneler SÖKÜLÜ · RC kumanda bağlı (PreArm "Radio
   failsafe on" çözülmeden ARM olmaz) · QGC laptop + MicoAir çifti ·
   fcu_url=ttyUSB0:57600 (USB-C soketi tamirde).
2. **Sıra:** M1 hızlı tekrar → M2 (QGC↔MicoAir) → M3 (QGC'den görev upload,
