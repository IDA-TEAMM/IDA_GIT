...
|---|---|---|
| (1) İDA↔YKİ kablosuz; İDA bilgisi YKİ'de | §3.2 QGC bağlantısı | QGC ↔ MicoAir ↔ Pixhawk (MAVROS'tan bağımsız kanal) |
...
(MicoAir MAVLink taşır, IP taşımaz) → md 3.3.1(3) "YKİ/RC'den komutla
...
(`fsm_node:311`), QGC'nin arm butonu MicoAir üzerinden yeter.
...
- [ ] QGC ↔ MicoAir ↔ Pixhawk zinciri masada teyit: telemetri görünüyor,
...
3. **QGC bağlantısı:** MicoAir linki yeşil, telemetri akıyor (md 3.3.1(1)).
...
1. Jetson'dan al: `~/girdap_logs/grafik/grafik_<UTC>.csv` (Ekran-2 verisi;
   `telemetry_node` 10 Hz — T0-g `08f3fb5`).
2. Ekran-2 panelini üret:
   `python scripts/run_ekran2.py --mp4 --t0 <görev_başı_s> --t1 <görev_sonu_s>`
