...
|---|---|---|---|
| 1 | İDA↔YKİ **kablosuz** bağlantı; İDA bilgileri YKİ'de gösterilecek | YKİ=QGC, Pixhawk'a MicoAir ile doğrudan bağlanır (MAVROS'tan bağımsız 2. MAVLink kanalı; `gcs_url:""` doğru). QGC telemetri gösterir. | ⚠ saha (MicoAir çifti + QGC laptop) |
...
```
QGC (MicoAir) ──MAVLink──> Pixhawk görev hafızası
                              │ (USB ttyACM0)
