...
def test_no_spurious_p1_to_p2_when_parkur2_absent(ros_context, tmp_path) -> None:  # noqa: ANN001
    """BULGU 1 repro (Yahya sonkodv3 denetimi, 2026-07-15): parkur-2 HİÇ
    yokken (tek parkurlu görev = VİDEO senaryosu) parkur-1'in son waypoint'ine
    varış koşulsuz dist_to_last_wp_p1=0 besliyordu → MissionFSM sahte PARKUR2'ye
    geçiyor → Dosya-2'ye (md 4.2) gerçekte olmayan PARKUR2 satırı yazılıyordu.
    Düzeltme: sinyal yalnız gerçekten bir parkur-2 varsa beslenir
    (ParkurTransitionLogic'in kendi guard'ıyla tutarlı)."""
    ...
    try:
        ...
        node._on_tick()
        assert node._fsm.state is MissionState.PARKUR1, (
            "parkur-2 hiç yokken son waypoint'e varış sahte PARKUR2 tetikledi "
            "(BULGU 1)"
        )
        node._on_mission_complete(Bool(data=True))       # gerçek bitiş sinyali
        node._on_tick()
