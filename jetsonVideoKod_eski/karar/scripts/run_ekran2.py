...
Grafik CSV'sinden (telemetry_node, ~/girdap_logs/kayit/<N>/grafik.csv —
16.07 kayıt düzeni: her kayıt kendi numaralı klasöründe, ARM yenisini açar)
...
_DEFAULT_GRAPH_DIR = Path.home() / "girdap_logs" / "kayit"
...
def main() -> None:
    ...
    # B2: telemetry setpoint_source="fc" ise thrust sütunları FC servo çıkışının
    # YÜZDESİdir (N değil) — eksen etiketi kaynakla aynı olmalı, yoksa grafik
    # yanlış birim gösterir (md 3.3.1.1 dürüstlük).
    parser.add_argument(
        "--thrust-birim", default="N", choices=["N", "%"],
        help="thrust ekseni birimi: N (girdap/MPPI) | %% (fc/AUTO video)",
    )
    ...
    # Kayıt düzeninde dosya adı hep "grafik.csv" → çıktıyı kayıt numarasıyla
    # adlandır (ekran2_kayit3.png); eski düzen adları stem'ini korur.
    stem = (
        f"kayit{csv_path.parent.name}"
        if csv_path.parent.name.isdigit() else csv_path.stem
    )
    out = (
        Path(args.out).expanduser() if args.out
        else Path.home() / "girdap_logs" / "viz" / f"ekran2_{stem}.{ext}"
    )
    if args.mp4:
