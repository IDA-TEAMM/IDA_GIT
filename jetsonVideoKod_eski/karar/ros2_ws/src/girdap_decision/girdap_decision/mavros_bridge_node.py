...
class MavrosBridgeNode(Node):
    ...
    def __init__(self, **node_kwargs) -> None:
        ...
        # F-M.4: FCU mod isteğini reddederse retry'lar arası asgari bekleme
        # (sn). 0 → sınır kapalı. Fix'siz masa senaryosunda 10 Hz istek
        # yağmurunu keser; sahada fix ARM'dan önce geldiği için tek istekte biter.
        self.declare_parameter("mode_retry_interval_s", 2.0)
        ...
        cfg = MavrosBridgeConfig(
            ...
            mode_retry_interval_s=float(
                self.get_parameter("mode_retry_interval_s").value
            ),
        )
    ...
    def _now(self) -> float:
        # F-M.10: heartbeat/retry yaşları GÖRELİ süre — duvar saati (get_clock)
        # NTP sıçramasında yaş gibi görünür (16.07: +7774 sn → sahte KILL).
    ...
    def _maybe_auto_guided(self) -> None:
        ...
        F-M.4: FCU reddedince (GPS fix'siz GUIDED) her state mesajı yeni
        istek üretiyordu → çekirdek retry hız sınırı uygular. Sayaç KARAR
        anında yakılır (servis o an hazır değilse retry en fazla bir aralık
        gecikir — state mesajları mavros'tan geldiği için servis pratikte
        hazırdır; F-M.6'daki ertelenen-istek deseninden bilinçli sapma).
        """
        if (
            self._auto_guided
            and not self._mode_req_pending
            and self._bridge.should_request_mode(self._now())
        ):
            self._bridge.note_mode_requested(self._now())
