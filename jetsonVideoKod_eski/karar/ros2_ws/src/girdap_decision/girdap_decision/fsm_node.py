...
class FSMNode(Node):
    ...
    def _on_waypoint_reached(self, msg: Int32) -> None:
        ...
        """
        idx = int(msg.data)
        # BULGU 1: gözlem yalnız GERÇEKTEN bir parkur-2 varsa beslenir. Tek
        # parkurlu görevde (video: 4 nokta) son waypoint'e varış aksi halde
        # MissionFSM'i sahte PARKUR2'ye geçirir → Dosya-2'ye (md 4.2) hiç
        # koşulmayan bir parkur satırı yazılır. ParkurTransitionLogic zaten
        # kendi geçişini parkur varlığına göre kapılıyor — burası da tutarlı.
        last_p1 = self._parkur.last_index_of_parkur.get(1)
        if idx == last_p1 and 2 in self._parkur.last_index_of_parkur:
